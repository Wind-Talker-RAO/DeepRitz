...
class DeepRitzSolver:
    ...
    def energy_loss(self, interior_points, boundary_points=None):
        """
        ...
        if boundary_points is not None:
            ...
            u_exact_bnd = self.exact_solution(x_bnd)
            #print(u_exact_bnd)
    ...
    def exact_solution(self, x):
        ...
        x_coord = x[:, 0:1]
        y_coord = x[:, 1:2]
        return torch.sin(np.pi * x_coord) * torch.sin(np.pi * y_coord)
    
    def train(self, num_epochs, interior_points, boundary_points=None):
        losses = []
        
        for epoch in range(num_epochs):
            self.optimizer.zero_grad()
            
            loss = self.energy_loss(interior_points, boundary_points)
            loss.backward()
            self.optimizer.step()
...
class P:
    def __init__(self, hidden_dim=50, num_layers=4, lr=0.001, num_epochs=5000, num_interior=1000, num_boundary=400):
...
def main(P):
    ...
    interior_points, boundary_points = generate_training_data(P.num_interior, P.num_boundary)
    
    # Initialize network and solver
    net = PoissonNet(hidden_dim=P.hidden_dim, num_layers=P.num_layers)
    solver = DeepRitzSolver(net, lr=P.lr)
    
    # Train the model
    print("Training started...")
    losses = solver.train(P.num_epochs, interior_points, boundary_points)
    ...
    u_exact = solver.exact_solution(test_points)
